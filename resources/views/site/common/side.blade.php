	...
	<ul class="topgame">
		...
		<li style="background-image: url('{!! $value->image !!}');">
			<div class="topgame-item">
				<a href="{{ CommonUrl::getUrl($value->url) }}">{!! $value->name !!}</a>
			</div>
		</li>
		@endforeach
	</ul>
...
<div class="side">
	...
	<ul class="menu vertical sidemenu">
		...
		<li {{ checkCurrent(url($value->url)) }}><a href="{{ CommonUrl::getUrl($value->url) }}" class="hvr-icon-float-away">{!! $value->name !!}</a></li>
		@endforeach
	</ul>
	<div class="see-all">
...
<div class="side">
	...
	<ul class="tagmenu">
		...
		<li><h2><a href="{{ CommonUrl::getUrl($value->url) }}" title="{!! $value->name !!}">{!! $value->name !!}</a></h2></li>
		@endforeach
	</ul>
</div>
@endif